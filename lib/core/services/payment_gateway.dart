...
abstract class PaymentGateway {
  Future<PaymentResult> processPayment({required double amount, required String currency});
}

class MockStripeGateway implements PaymentGateway {
  @override
  Future<PaymentResult> processPayment({required double amount, required String currency}) async {
...
class PaymobGateway implements PaymentGateway {
  ...
  int get cardIntegrationId => int.tryParse(dotenv.env['PAYMOB_INTEGRATION_ID'] ?? '') ?? 0;
  String get iframeId => dotenv.env['PAYMOB_IFRAME_ID'] ?? '';

  PaymobGateway({required this.dio, required this.firebaseAuth, String? functionsBaseUrl});

  Future<String> _getAuthToken() async {
    AppLogger.info('[Paymob] Step 1: Getting authentication token...');

    final response = await dio.post('$paymobBaseUrl/auth/tokens', data: {'api_key': apiKey});

    final token = response.data['token'] as String;
    AppLogger.success('[Paymob] Auth token received: ${token.substring(0, 20)}...');
    return token;
  }
  ...
  Future<String> _getPaymentKey({
    ...
    AppLogger.success('[Paymob] Payment key received: ${paymentToken.substring(0, 20)}...');
    return paymentToken;
  }
  ...
  Future<PaymentResult> processPayment({required double amount, required String currency}) async {
    ...
    final firstName = nameParts.isNotEmpty && nameParts.first.isNotEmpty ? nameParts.first : 'User';
    final lastName = nameParts.length > 1 && nameParts.last.isNotEmpty ? nameParts.last : 'Name';

    final billingData = {
      'apartment': 'NA',
      'email': firebaseAuth.currentUser?.email ?? 'user@test.com',
      'floor': 'NA',
    ...
    final paymentUrl = '$paymobBaseUrl/acceptance/iframes/$iframeId?payment_token=$paymentToken';
    ...
    return PaymentResult(paymentUrl: paymentUrl, orderId: paymobOrderId.toString());
  }
}
