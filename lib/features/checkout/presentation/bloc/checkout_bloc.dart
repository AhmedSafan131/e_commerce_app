  ...
  Future<void> _onProcessPayment(ProcessPaymentEvent event, Emitter<CheckoutState> emit) async {
    ...
    try {
      ...
      if (paymentResult.isLeft()) {
        final failure = paymentResult.swap().getOrElse(() => const ServerFailure('Payment error'));
        emit(state.copyWith(status: CheckoutStatus.failure, errorMessage: failure.message));
        return;
      }

      // Step 2: Extract payment URL and order ID
      final paymentData = paymentResult.getOrElse(() => throw Exception('Payment result is null'));
